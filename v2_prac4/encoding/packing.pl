...
writeClauses:-
    onlyOneStartInArea,
    forAllStartFillUponArea,
    noOverlapping,
    true.
    
onlyOneStartInArea:-
    rect(B),
    findall(start-B-X-Y, insidePiece(B,X,Y), Lits),
    atLeast(1,Lits),
    %debug(Lits),
    fail.
onlyOneStartInArea.   
...
forAllStartFillUponArea:-
    rect(B),
    insidePiece(B,X,Y,Bx,By),
    writeClause([\+start-B-X-Y, fills-B-Bx-By]),
    %debug([\+start-B-X-Y, fills-B-Bx-By]),
    fail.
forAllStartFillUponArea.
...
displaySol(M):-
    ...
    write(Yp), 
    write(":-\t"),
    printSolutionLine(M,Yp), 
    nl,
    fail.
    
displaySol(_).

printSolutionLine(M,Y):-
    insideTable(X,Y),
    printNum(X,Y,M),
    fail.
printSolutionLine(_,_).
