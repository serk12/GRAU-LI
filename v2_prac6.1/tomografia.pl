...
unifyCols(0, L, [],L).
unifyCols(N, [X|L], [X|Row], Rest):- N > 0, N1 is N-1, unifyCols(N1,L,Row,Rest).

declareConstraints([],[]).
declareConstraints([List|Rest],[ListSum|RestSum]):- 
    sum(List, #=, ListSum), 
    declareConstraints(Rest,RestSum).
