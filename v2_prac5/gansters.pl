...
%% of the tasks to be done every hour.
    ...
    %% No gangster can do two different tasks during the same hour or on two consecutive hours.     [X]

    %% Some gangsters are not available on certain hours.                                           [X]
    
    %% We want to plan all tasks (which gangster does what task when).                              [X]

        %% we want to find the minimal K such that no gangster works more than K consecutive hours. [X]
...
noConsecutiveDiferentTask:-
    ...
    task(T2),
    T2 \= T1,
    writeClause([\+does-G-T1-H1, \+does-G-T2-H2]), %A -> ¬B  === ¬A u ¬B
    fail.
noConsecutiveDiferentTask.
...
noConsecutiveWork(K):- 
    task(T),
    available(G,HIni), HFin is HIni+K, hour(HFin),
    findall(\+does-G-T-H,between(HIni,HFin,H),Lits),
    writeClause(Lits),
    fail.
noConsecutiveWork(_).
...
maxConsecutiveHours(M,K):-
    ...
    sort(L,Ls),
    subVect(Ls,X,Xs),
    consecutiveNums(Xs,X,K),
    !.
consecutiveNums([],_,1).
consecutiveNums([X|Xs],Y,K):-
    Y is X-1,
    T is K-1,
    consecutiveNums(Xs,X,T).
subVect([X|Xs],X,Xs).
subVect([_|Xs],A,B):- subVect(Xs,A,B).
